...
function MessageCenter({ groupName, groupImg, groupId }: MessageCenterProps) {
  const { toggleBoxReply, toggleBoxSearchMessage, setToggleBoxSearchMessage } = useConversationStore()
  ...
  useEffect(() => {
    if (toggleBoxReply || toggleBoxSearchMessage) {
      ...
      if (boxSearchRef.current) {
        console.log(boxSearchRef.current?.getBoundingClientRect())
  ...
  }, [toggleBoxReply, toggleBoxSearchMessage])
  ...
  return (
    <div className='h-full flex-1'>
      {/* chat heading */}
      <div className='w- uk-animation-slide-top-medium relative z-10 flex flex-col items-center justify-between gap-2 border-b px-6 py-3.5 dark:border-slate-700'>
